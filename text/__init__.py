...
def phoneme_to_sequence(text,  enable_eos_bos=False, tp=None):
    # pylint: disable=global-statement
    ...
    # print(_phonemes_to_id)

    for phoneme in text:
        sequence.append(_phoneme_to_sequence(phoneme))
    # Append EOS char
    if enable_eos_bos:
...
def _phoneme_to_sequence(phons):
    if _should_keep_phoneme(phons):
        return _phonemes_to_id[phons]
...
def _should_keep_phoneme(p):
    return p in _phonemes_to_id.keys() and p not in ['~', '^', '_']
